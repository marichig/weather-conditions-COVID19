% a set of systematic tests to assess different parameterizations.

clear all

testNum=20; %number of iterations per experiment
numC=1000;  %number of cities per test
tm=[0 -0.01 -0.002]; %the baseline functional form with constant, first order, and quadratic effects
dirNm='C:\Users\hazhi\Dropbox (MIT)\COVID-19\Data analysis\EstimatingDelay\syntheticDataUpdate\FinalPaperReported';
...
cmlDifFunc=0;   % test performance in different functional forms with cumulative exposure estimation
...
load('RealData');
D=zeros(size(impD{2},1)+size(impDW{2},1),min(size(impD{2},2),size(impDW{2},2)));
%combine World and US temperature data into a single file
D(1:size(impD{2},1),:)=impD{2}(:,1:size(D,2));
D(size(impD{2},1)+1:end,:)=impDW{2}(:,1:size(D,2));
...
if trueInf
    ...
    for i=1:3
        ...
        xlswrite(xlsFN,[sz rsqr],'Sheet1',[xlcl{i} num2str(row+2)]);
    end
    row=row+6;
end
...
if cmlDifFunc
    xlswrite(xlsFN,{'performance in different functional forms with cumulative exposure estimation and detection fraction of 1'},'Sheet1',[xlcl{1} num2str(row-2)]);
    for i=1:3
        switch i
            case 1
                rvar=[0 +0.05 0];
